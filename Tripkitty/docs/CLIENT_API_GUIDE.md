...
| `422 Unprocessable Entity` | `SELF_REQUEST`, `INVALID_PAYER`, `INVALID_SHARE`, `INVALID_SPONSORS`, `USER_NOT_FOUND`, `WRONG_PASSWORD`, `INVALID_TOKEN`, `VERSION_CONFLICT`, `INVALID_PHONE`, `INVALID_BANK`, `SPONSOR_SELF` |
...
```json
{
  ...
  "expenses":[ { "id": "...", "title": "Такси", "amount": 1200.50, "payer": "u_...", "share": [{"participantId":"u_..."},{"participantId":"g_..."}], "splitType": 0, "createdBy": "u_...", "isTransfer": false, "sponsors": {} } ],
...
плательщик (`payer`), в чьём-либо `share`, или как спонсор в карте `sponsors`
расхода (§4.4), в котором подопечный реально участвует. В этом случае ответ — `409 Conflict`,
...
**Спонсорство действует по-расходно, а не глобально.** Флаг на участнике — это
**дефолт для новых расходов**: при создании расхода сервер снапшотит текущую карту
спонсорства поездки в поле `sponsors` расхода (`{подопечный: спонсор}`), и дальше
эта карта живёт на расходе своей жизнью:

- **Включение/снятие флага не трогает уже внесённые расходы** — они сохраняют свой
  снапшот. Снял бюджет в середине поездки → старые расходы остались на спонсоре,
  новые подопечный платит сам.
- **Точечное исключение**: у конкретного расхода пару можно убрать (или вернуть)
  через PATCH расхода с полем `sponsors` (§5.1–5.2) — «жена шопилась сама, этот
  расход не на мне».
- В расчёте (§5.4) доля и платёж подопечного в расходе, где пара записана,
  зачисляются спонсору; в расходах без пары подопечный участвует сам за себя.
  В переводы по покрытым расходам подопечный не попадает.
...
- `sponsors` (опционально) — карта общего бюджета для этого расхода
  `{ "подопечныйId": "спонсорId" }`, см. §4.4. **Не передан (`null`)** — сервер сам
  снапшотит текущее спонсорство поездки (обычный случай: клиент вообще не думает
  про это поле). **Передан** — используется как есть; разрешены только пары,
  совпадающие с живым спонсорством поездки, иначе `422 INVALID_SPONSORS`
  (field `sponsors`). Пустой объект `{}` — «в этом расходе каждый сам за себя».
...
`sponsors` — снапшот общего бюджета этого расхода (§4.4): чья доля/платёж в этом
расходе зачисляется спонсору. Пустой объект — никто никого не покрывает.
...
Единственное исключение — `sponsors`: **не передан (`null`)** = оставить карту
расхода как есть (в отличие от создания, где `null` = снапшот живого спонсорства).
Передан — заменяет карту целиком; разрешены пары из живого спонсорства поездки
**плюс** пары, уже записанные на этом расходе (то есть снятую галочку можно вернуть,
даже если общий бюджет в поездке уже выключен). Чужие пары — `422 INVALID_SPONSORS`.
...
- `balances` — итоговый баланс каждого участника: **положительный** = ему должны,
  **отрицательный** = должен он. Считается **с учётом общих бюджетов** (§4.4):
  доля/платёж подопечного в расходах, где записана пара `sponsors`, зачислены
  спонсору. Подопечный, покрытый во всех своих расходах, имеет здесь `0`; если
  часть его расходов не покрыта (галочку сняли) — остаток висит на нём самом.
- `ownBalances` — персональные балансы **до** переливаний спонсорам. Без спонсоров
  совпадает с `balances`. Используйте для прозрачности: «сколько из долга спонсора —
  траты подопечного» (`ownBalances[dependent] - balances[dependent]`); в UI
  подопечных показывайте внутри блока спонсора, а не как «ничего не должен».
